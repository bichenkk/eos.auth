...
import { keys } from '../../actions/permissionForm'
  ...
  render() {
    ...
    return (
      <Form onSubmit={this.handleOnSubmit}>
        ...
        <FormItem label='Permission Name' {...formItemLayout}>
          {getFieldDecorator('title', {
            rules: [{ required: true, whitespace: true, message: 'Please input a valid value.' }],
          })(<Input />)}
        </FormItem>
        <FormItem label='Tag Color' {...formItemLayout}>
          {getFieldDecorator('color', {
            rules: [{ required: true, whitespace: true, message: 'Please input a valid value.' }],
          })(<ColorPicker />)}
        </FormItem>
        <FormItem label='Device' {...formItemLayout}>
          {getFieldDecorator('device', {
            initialValue: 'Synology NAS',
            rules: [{ required: true, whitespace: true, message: 'Please input a valid value.' }],
          })(<DeviceTypeSelect />)}
        </FormItem>
        <FormItem label='Data Path' {...formItemLayout}>
          {getFieldDecorator('data_path', {
            initialValue: 'volume1/private/',
            rules: [{ required: true, whitespace: true, message: 'Please input a valid value.' }],
          })(<Input />)}
        </FormItem>
        {
